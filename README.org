...
For Linux, download the [[file:qmk-firmware/][qmk_firmware]] folder from this repository, then run
#+begin_src sh
sudo emerge dev-vcs/git dev-python/pip # for gentoo or
sudo apt install -y git python3-pip # for ubuntu
python3 -m pip install --user qmk
cd /path/to/the/folder/qmk_firmware/
./bin/qmk setup
#+end_src
...
**** OPTION 2: Using QMK Configurator for same parts
...
#+begin_src sh
./bin/qmk compile -kb handwired/dactyl_manuform_r_track -km <your-keymap>
#+end_src
...
*** Install avrdude
On Gentoo this is ~sudo emerge avrdude~, on Ubuntu ~sudo apt install avrdude~ and I am confident someone nerdy enough to want to use such a keyboard knows how to install software on his/ her distro.
