...
Unlike some other trackball Dactyl Manuforms that utilize a [[https://www.pjrc.com/store/teensy.html][Teensy 2.0]] I managed to cramm the firmware onto a Arduino Micro. Be aware that there is couple of hundred bytes left for hacking about. So if you want to add more custom stuff into your trackball Dactyl this one is probably not the best choice for you.
...
- [[#building-guide-note-wip][Building guide NOTE: WIP]]
  ...
  - [[#wiring--mounting][Wiring & Mounting]]
    - [[#try-mounting-the-usb-c-to-micro-usb-cable-and-the-trrs-socket][Try mounting the USB-C to micro-USB cable and the TRRS socket.]]
    ...
    - [[#solder-the-jumper-wires-between-the-matrix-and-the-arduino-and-between-the-trrs-socket-and-the-arduino][Solder the jumper wires between the matrix and the Arduino and between the TRRS socket and the Arduino.]]
...
Now that the PMW3360 and the TRRS socket is connected, you will notice that there is no GND pin left on the Arduino. So solder one of the wires coming from the switch to the GND pin you used on the TRRS socket. Connect the other wire to any of the two reset pins of the Arduino.
